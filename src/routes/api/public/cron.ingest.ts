// Daily INGEST cron. Runs at 01:30 UTC, hours before the drop, so that every
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const ranked = [...all]
          .sort((a, b) => importanceOf(b.f) - importanceOf(a.f))
          .slice(0, TOP_N);
        ...
        for (const { lg, f } of ranked) {
          ...
          const byTeam = (id: number) =>
            (st.find((x: Json) => x.team?.id === id) ?? {}).statistics ?? [];
        ...
        for (const { lg, f } of ranked) {
          ...
          const res = await af(
            `/fixtures/headtohead?h2h=${f.teams.home.id}-${f.teams.away.id}&last=10`,
          );
          const meetings = res
        ...
        if (FD_KEY) {
          for (const lg of live) {
            ...
            try {
              ...
              if (r.ok)
                fdList =
                  ((await r.json()) as Json).matches?.filter(
                    (m: Json) => m.status === "FINISHED",
                  ) ?? [];
            } catch {
              fdList = null;
            }
            ...
            for (const { f } of mine) {
              ...
              const ok =
                hit.score?.fullTime?.home === f.goals.home &&
                hit.score?.fullTime?.away === f.goals.away;
              ctx.feeds_agree = ok;
              if (ok) agreed++;
              else {
                disagreed++;
