...
- **Access ladder + waitlist (2026-07-06, `15-access-and-waitlist-plan.md`).** Anonymous: recent drops + two pundits (The Reporter, The Gaffer; pick in localStorage under `ft-voice-style`). Free account: all six pundits (server gate on `setVoiceStyle` is signed-in), synced settings. Waitlist for the full app: `waitlist` table (own-row RLS + guard trigger), `/waitlist` page with one-tap join or magic-link-is-the-join for anonymous, CTAs on Today + Settings. Files: `src/lib/entitlement.ts`, `src/lib/api/waitlist.functions.ts`, `src/routes/waitlist.tsx`.
- **Billing + Pro: PARKED.** The Stripe plumbing ($4.99/mo, TEST key, webhook, DB guard trigger) stays wired but gates nothing user-visible; `/pro` redirects to `/waitlist`. Files: `src/lib/api/billing.functions.ts`, `src/routes/api/stripe/webhook.ts`, `src/lib/stripe.server.ts`, `src/lib/billing-sync.server.ts`, `src/hooks/use-entitlement.ts`.
...
- Anonymous listeners get two pundits (The Reporter, The Gaffer); the other four need a free account, enforced in the UI and by auth on the server `setVoiceStyle`. Pro gates nothing right now (parked). The `profiles` billing columns stay service_role-only regardless.
