...
- **Full Time Pro, $4.99/mo USD: STILL PARKED, but now on live Stripe keys (2026-08-07).** Production holds the live secret, the live price (`price_1Tpo8W...`, $4.99/mo USD) and a live webhook; preview and development stay on the test key so a preview deploy can never transact. What has NOT changed is the product decision: `/pro` still 307s to `/waitlist`, so **there is no user-reachable way to pay**. The tier returns when there are features worth paying for. We never call an unbuilt thing done; "rolling out" copy stays until each thing ships.
